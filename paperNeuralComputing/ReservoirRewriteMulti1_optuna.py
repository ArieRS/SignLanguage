...
def main_function(trial):
    ...
    configParam['n_internal_units'] = trial.suggest_int("n_internal_units", 300, 500, log=True) 
    configParam['spectral_radius'] = [trial.suggest_float("spectral_radius", 0.1, 1, log=True)] 
    configParam['leakList'] = [trial.suggest_float("leakList1", 0.1, 1, log=True)] 
    configParam['connectivity'] = [trial.suggest_float("connectivity", 0.1, 1, log=True)] 
    configParam['input_scaling'] = trial.suggest_float("input_scaling", 0.1, 1, log=True)
    configParam['noise_level'] = trial.suggest_float("noise_level", 0.1, 1, log=True)  
    configParam['n_drop'] = 0  # transient states to be dropped
  
    configParam['w_ridge_embedding'] = trial.suggest_int("w_ridge_embedding", 1, 50, log=True)
    # Type of readout
    configParam['readout_type'] = 'lin'  # readout used for classification: {'lin', 'mlp', 'svm'}
    # Linear readout hyperparameters
    configParam['w_ridge'] = trial.suggest_int("w_ridge", 1, 50, log=True)
