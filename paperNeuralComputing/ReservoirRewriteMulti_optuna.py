...
def main_function(trial):
    ...
    configParam['n_internal_units'] = 50  # trial.suggest_int("n_internal_units", 33, 170, log=True) 
    configParam['spectral_radius'] = [0.177, 0.177, 0.177]  # [trial.suggest_float("spectral_radius", 0.1, 1, log=True)] 
    configParam['leakList'] = [0.6834,  # trial.suggest_float("leakList1", 0.1, 1, log=True) 
                               0.8007,  # trial.suggest_float("leakList2", 0.1, 1, log=True)
                               trial.suggest_float("leakList3", 0.1, 1, log=True)]
    ...
    meanAcc, stdAcc = np.mean(listAccuracy), np.std(listAccuracy)
    meanF1, stdF1 = np.mean(listF1), np.std(listF1)
    print(f'{maxAccuracy =} ; {meanAcc =}; {stdAcc =} ; {meanF1 =}; {stdF1 =} ;{meanTrainingTime} ; {meanTestingTime}')
    '''  '''
